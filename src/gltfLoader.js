...
GLTFLoader.prototype.parse = function(data, path, onLoad, onError) {
  ...
  if (json.extensionsUsed) {
    ...
    if (
      json.extensionsUsed.indexOf(EXTENSIONS.KHR_DRACO_MESH_COMPRESSION) >= 0
    ) {
      extensions[
    ...
    for (var i = 0, il = animationDef.channels.length; i < il; i++) {
      ...
      if (sampler) {
        ...
        if (node) {
          ...
          var input = new inputAccessor.array.constructor(
            inputAccessor.array.subarray(0, inputAccessor.array.length)
          );
          var output = new outputAccessor.array.constructor(
            outputAccessor.array.subarray(0, outputAccessor.array.length)
          );
      ...
      for (var i = 0, il = primitives.length; i < il; i++) {
        ...
        if (useVertexColors) {
          switch (geometry.attributes.a_Color.size) {
            ...
            default:
              console.warn(
                'Illegal vertex color size: ' + geometry.attributes.a_Color.size
              );
          }
        }
  ...
  return Promise.all(pendingBufferViews).then(function(bufferViews) {
    ...
    if (accessorDef.sparse !== undefined) {
      ...
      for (var i = 0, il = sparseIndices.length; i < il; i++) {
        ...
        bufferAttribute.array[index * bufferAttribute.itemSize] =
          sparseValues[i * itemSize];
        if (itemSize >= 2)
          bufferAttribute.array[index * bufferAttribute.itemSize + 1] =
            sparseValues[i * itemSize + 1];
        if (itemSize >= 3)
          bufferAttribute.array[index * bufferAttribute.itemSize + 2] =
            sparseValues[i * itemSize + 2];
        if (itemSize >= 4)
          bufferAttribute.array[index * bufferAttribute.itemSize + 3] =
            sparseValues[i * itemSize + 3];
        if (itemSize >= 5)
          throw new Error(
            'zen3d.GLTFLoader: Unsupported itemSize in sparse BufferAttribute.'
          );
      }
    }
...
GLTFParser.prototype.loadBuffer = function(bufferIndex) {
  ...
  if (bufferDef.type && bufferDef.type !== 'arraybuffer') {
    throw new Error(
      'zen3d.GLTFLoader: ' + bufferDef.type + ' buffer type is not supported.'
    );
  }
  ...
  return new Promise(function(resolve, reject) {
    loader.load(
      ...
      function() {
        reject(
          new Error(
            'zen3d.GLTFLoader: Failed to load buffer "' + bufferDef.uri + '".'
          )
        );
      }
    );
...
GLTFParser.prototype.loadMaterial = function(materialIndex) {
  ...
  if (materialExtensions[EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS]) {
    ...
    if (metallicRoughness.baseColorTexture !== undefined) {
      ...
      materialParams.diffuseMapCoord =
        metallicRoughness.baseColorTexture.texCoord || 0;
    }
  ...
  if (materialDef.emissiveFactor !== undefined) {
    materialParams.emissive = new Color3().fromArray(
  ...
  return Promise.resolve(sourceURI)
    ...
    .then(function(image) {
      ...
      if (
        ...
        console.warn(
          'zen3d.GLTFLoader: js does not support texture internalFormat which is different from texture format. ' +
      ...
      texture.type =
        textureDef.type !== undefined
          ...
          : WEBGL_PIXEL_TYPE.UNSIGNED_BYTE;

      var samplers = json.samplers || {};
      var sampler = samplers[textureDef.sampler] || {};

      texture.magFilter =
        WEBGL_FILTERS[sampler.magFilter] || WEBGL_TEXTURE_FILTER.LINEAR;
      texture.minFilter =
        WEBGL_FILTERS[sampler.minFilter] ||
        WEBGL_TEXTURE_FILTER.LINEAR_MIPMAP_LINEAR;
      texture.wrapS =
        WEBGL_WRAPPINGS[sampler.wrapS] || WEBGL_TEXTURE_WRAP.REPEAT;
      texture.wrapT =
        WEBGL_WRAPPINGS[sampler.wrapT] || WEBGL_TEXTURE_WRAP.REPEAT;
...
GLTFMaterialsPbrSpecularGlossinessExtension.prototype.extendParams = function(
  materialParams,
  material,
  parser
) {
  ...
  for (var lightId in lights) {
    ...
    if (lightNode) {
      ...
      if (light.fallOffExponent !== undefined) {
        console.warn(
          'GLTFLoader:: light.fallOffExponent not currently supported.'
        );
      }
...
GLTFTextureTransformExtension.prototype.extendTexture = function(
  ...
  if (transform.texCoord !== undefined) {
    console.warn(
      'zen3d.GLTFLoader: Custom UV sets in "' +
        this.name +
